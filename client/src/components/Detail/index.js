...
export function Detail(props){
    ...
    <div>
        <Nav/>
        <div>{props.detalle[0]?    <div className="detail">
            ...
            <div id ="altura">{props.detalle[0]? (props.detalle[0].height? <div>Altura aproximado: {props.detalle[0].height.metric} Cm</div>: props.detalle[0].altura?<div>Altura aproximado: {props.detalle[0].altura}</div>: <span>sin datos de altura</span>): <span>No hay perrito, lo siento</span>}</div>
        ...
        </div>: <img id="loading"src={loading} alt="cargando"/>}</div>
    </div>
    )
}
